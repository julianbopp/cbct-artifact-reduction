...
class LakeFSClient(S3Client):
    def __init__(
        ...
        self.local_cache_path_commit = os.path.join(
            self.local_cache_path, self.commit_id
        )
    ...
    def get_file(self, object_name, file_obj=None):
        ...
        if not os.path.exists(local_path):
            try:
                file = self.lakefs.get_object(
                    self.repo_name, self.commit_id, object_name
                )
...
class Boto3Client(S3Client):
    def __init__(
        ...
        self.local_cache_path_commit = os.path.join(
            self.local_cache_path, os.getlogin()
        )

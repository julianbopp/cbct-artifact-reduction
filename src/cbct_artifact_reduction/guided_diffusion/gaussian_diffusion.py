...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        x = x[:, 0:1, ...]  # Loss is only computed for the first channel
    ...
    def p_sample_loop_inpainting(
        ...
        noise = th.randn_like(masked_image).to(device)
        masked_image = masked_image.to(device)
        mask = mask.to(device)
        x_noisy = th.cat((noise, masked_image, mask), dim=1).float()

        shape = masked_image.shape

        final = None
        for sample in self.p_sample_loop_progressive_inpainting(
            model,
            shape,
            noise=x_noisy,
            clip_denoised=clip_denoised,
            ...
            final = sample
        if final is not None:
            return final["sample"], x_noisy
        else:
            return final, x_noisy
